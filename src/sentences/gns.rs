...
use super::utils::{number, parse_hms, parse_lat_lon};
...
fn do_parse_gns(i: &[u8]) -> IResult<&[u8], GnsData> {
    ...
    let (i, _) = char(',')(i)?;
    let (i, _) = take_until(",")(i)?; // TODO parse FAA
    let (i, _) = char(',')(i)?;
    ...
    let (i, _) = char(',')(i)?;
    let (i, _) = take_until(",")(i)?; // TODO parse age of diff. corr.
    let (i, _) = char(',')(i)?;
    let (i, _) = take_until(",")(i)?; // TODO parse diff. station ID
    let (i, _) = char(',')(i)?;
    let (i, nav_status) = map(one_of("SCUV"), |ch| match ch {
...
mod tests {
    ...
    fn test_parse_gns() {
        ...
        assert_eq!(NavigationStatus::Safe, gns_data.nav_status);
    }
}
